...
class CorporateBonds():
    ...
    def AAA(self, timeframe = str, observation_end=date.today().strftime("%m/%d/%Y"))->pd.DataFrame:
        ...
        if timeframe == 'Weekly':
            self.data = fred.get_series('WAAA', observation_start='01/05/1962', observation_end=observation_end)
        elif timeframe == 'Daily':
            self.data = fred.get_series('DAAA', observation_start = '01/01/1985', observation_end = observation_end)
        else:
            self.data = fred.get_series('AAA', observation_start='01/01/1919', observation_end=observation_end)
